...
- See also [Install scripts | baklava](https://ii.clsty.link/en/dev/inst-script/)
...
## Attentions
### PAM
On non-NixOS systems, programs using PAM (typically screen locker) may not work if installed via Nix, so the system-provided package may still be needed.

- One problem is that the Nix PAM integration may not support the local `/etc/pam.d` setup used by the system.
- Another problem is the location of a suid helper binary that is necessary.

As noted by @Cu3PO42, both problems could be solved by using the system-provided libpam instead.
...
```
_Note: it uses `systemctl`, so it may not work on every init system..._
