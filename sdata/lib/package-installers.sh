...
install-python-packages(){
  UV_NO_MODIFY_PATH=1
  BAKLAVA_VIRTUAL_ENV=$XDG_STATE_HOME/quickshell/.venv
  x mkdir -p $(eval echo $BAKLAVA_VIRTUAL_ENV)
  # we need python 3.12 https://github.com/python-pillow/Pillow/issues/8089
  try uv venv --prompt .venv $(eval echo $BAKLAVA_VIRTUAL_ENV) -p 3.12
  x source $(eval echo $BAKLAVA_VIRTUAL_ENV)/bin/activate
