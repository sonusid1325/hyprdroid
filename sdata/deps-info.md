...
Tips:
- The packages which name has prefix `baklava-` are defined with local files `PKGBUILD`. There're two types:
...
- For each package included in the local `PKGBUILD`s which name does **not** have prefix `baklava-`, for example `rsync`, it's either from [Arch Linux Packages](https://archlinux.org/packages) or the [AUR](https://aur.archlinux.org/packages). Search the package name on them to get the info (e.g. what executable(s) the package provides).
...
# Actual packages
## baklava-quickshell-git
- Pinned commit.
- Also with extra dependencies (mainly Qt things) needed by the baklava Quickshell config.
