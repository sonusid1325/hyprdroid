...
For summary:
- They will be installed to the virtual environment `$BAKLAVA_VIRTUAL_ENV`.
- The default value of `$BAKLAVA_VIRTUAL_ENV` is `$XDG_STATE_HOME/quickshell/.venv`.
  - The default value of `$XDG_STATE_HOME` is `$HOME/.local/state`.
- Currently we use `env = BAKLAVA_VIRTUAL_ENV, ~/.local/state/quickshell/.venv` in `~/.config/hypr/hyprland/env.conf` to set this environment variable.[^2]
...
```bash
source $(eval echo $BAKLAVA_VIRTUAL_ENV)/bin/activate
```
...
```python
#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\$BAKLAVA_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
```
And that's it!
